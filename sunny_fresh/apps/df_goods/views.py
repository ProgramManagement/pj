...
=======
        'title': '梁非凡',
>>>>>>> 22cae4fba9db5d21930991d8b9324083856caa98
        'cart_num': cart_num,
        'guest_cart': 1,
...
def coupon(request):
    ...
    ulotterycount=user.ulotterycount

    context={
        'uconsumed':uconsumed,
        'ulotterycount':ulotterycount
    }
...
def do_coupon(request):
    ...
    user_id = request.session["user_id"]
    user = UserInfo.objects.get(pk=int(user_id))
    user.ulotterycount=user.ulotterycount-1   #每次抽奖无论结果，减少一次抽奖次数
    ...
    user.save()
    # context = {
    #     'new_lotterycount': user.ulotterycount
    # }
    #
    # response = render(request, 'df_goods/coupon.html',context)
    # return response
    data = {}
    data['lotterycount']=user.ulotterycount
